from dane_workflows.util.status_util import ProcessingStatus, StatusRow
...
def new_batch(source_batch_id: int, status: ProcessingStatus):
    ...
    return [
        StatusRow(
            target_id=x,  # Use this to reconsile results with source catalog (DANE.Document.target.id)
            ...
            proc_id=-1,  # ID assigned by the DataProcessingEnvironment
